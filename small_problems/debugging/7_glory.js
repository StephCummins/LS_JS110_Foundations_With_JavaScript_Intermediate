...
The error with the original code is found in the switch statement, within the
`targetRoll` function. Since a roll of 16 results in the default, the final
code block executes as intended. For cases 1 and 20, there is no break 
statement to separate them. A roll of 1 falls through, revealing the message
from case 1, case 20, and the default. A roll of 20 also falls through, 
revealing the message from case 20 and the default. A code fix is in the
solution above, with the break statement added to each case of the switch
statement.
